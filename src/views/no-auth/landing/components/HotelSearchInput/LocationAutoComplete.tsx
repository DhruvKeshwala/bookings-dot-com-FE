...
const CustomOption = (props: OptionProps<LocationOption, false>) => {
  ...
  return (
    <div
      {...innerProps}
      className={cn(
        "cursor-pointer bg-white p-4 flex text-black items-center gap-3 font-nunito w-full",
        isFocused && "bg-highlight text-primary"
      )}
    >
      <HotelIcon className="text-gray-500 size-5" />
      <div>
        <h4 className="font-medium opacity-80 font-nunito font-sm">
          {data.city}
        </h4>
        <p className="text-sm font-medium opacity-80 font-nunito font-xs">
          {data.country}
        </p>
      </div>
    </div>
...
const LocationAutoComplete = ({
  ...
  const fetchAndCacheLocations = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URI}/hotels/all-cities`
      );
  ...
  const selectedValue =
    options.find((c) => c.city === selectedLocation?.city) ||
    selectedLocation ||
    null;
  ...
  return (
    <div>
      <Select
        ...
        className="bg-transparent outline-none body-text text-[#646464]"
