...
const IataAutoComplete = ({
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setInputValue(
        parsed.airport_name
          ? `${parsed.airport_name} (${parsed.airport_code || parsed.code || ""})`
          : parsed.city
      );
    } else {
  ...
  return (
    <div
      ref={wrapperRef}
      className={cn(
        "border-b border-black/40 flex flex-col font-nunito text-black justify-center",
        containerClassName
      )}
    >
      <div className="text-black text-xs lg:text-base font-medium opacity-60">
        {title}
      </div>
      ...
      <div className="relative">
        <input
          ref={inputRef}
          type="text"
          className="w-full bg-transparent outline-none pt-2 px-0 text-lg font-bold font-nunito text-black placeholder:font-bold"
          value={inputValue}
